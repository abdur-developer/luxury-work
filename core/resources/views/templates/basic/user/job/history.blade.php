                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex flex-wrap table-icon">
                                <a href="{{ !($job->status == Status::JOB_REJECTED || $job->status == Status::JOB_COMPLETED) ? route('user.job.edit', $job->id) : 'javascript:void(0)' }}" class="text--success me-2 {{ $job->status == Status::JOB_REJECTED || $job->status == Status::JOB_COMPLETED ? 'disabled' : '' }} " data-bs-toggle="tooltip" title="Edit">
